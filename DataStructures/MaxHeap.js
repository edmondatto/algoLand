/* A Class that represents a Max Heap */
class MaxHeap {
  constructor() {
    this.items = []
  }
  ...
  /**
   * Returns the maximum value and removes it from the the heap
   * @return {number}
   */
  poll() {
  ...
  /**
   * Alters the heap from the bottom up if necessary, to ensure it follows the rules of a max heap
   */
  heapifyUp() {
  ...
  /**
   * Alters the heap from the top down if necessary, to ensure it follows the rules of a max heap
   */
  heapifyDown() {
    let index = 0
    while (this.hasLeftChild(index) && (this.items[index] < this.getLeftChild(index) || this.items[index] < this.getRightChild(index))) {
      ...
      if (this.hasRightChild(index)) {
        biggerChildIndex = this.getLeftChild(index) < this.getRightChild(index) ? this.getRightChildIndex(index) : biggerChildIndex
      }
